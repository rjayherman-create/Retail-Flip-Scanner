...
const seedInventory: Item[] = [
  ...
  { id:6,name:"Pitch Black Binacle 3-Pack",type:"3-Pack Blister",qty:1,cost:16.28,value:28,trend:9.8,signal:"WATCH",target:38,note:"Near target; wait for another $8–$10 of appreciation." },
...
function App(){
  ...
  const [scanMode,setScanMode]=useState<ScanMode|null>(null); const [selectedFiles,setSelectedFiles]=useState<File[]>([]); const [scanMessage,setScanMessage]=useState(""); const [scanning,setScanning]=useState(false);
  ...
  const startScan=async()=>{
    if(!selectedFiles.length){setScanMessage("Take a photo or choose a receipt/product file first.");return}
    if(scanMode==="receipt"){setScanMessage("Receipt OCR is separate. For this test, use Scan purchase with Pokémon product photos.");return}
    const images=selectedFiles.filter(file=>file.type.startsWith("image/")).slice(0,4);
    if(!images.length){setScanMessage("The Pokémon product scanner currently accepts JPG, PNG, HEIC, or other image files—not PDF files.");return}
    ...
    try{
      ...
      await loadPortfolio();
      setScanMessage(`${json.extracted?.product_name||"Pokémon product"} saved successfully. ${json.saved_items?.length||1} inventory unit created.`);
      setMessage("Pokémon scan completed and saved to PostgreSQL.");
