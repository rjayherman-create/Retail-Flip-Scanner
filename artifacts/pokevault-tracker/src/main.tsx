...
type ApiItem = {
  id: number; product_name: string; subcategory?: string | null;
  price?: number | null; current_store_price?: number | null;
  normal_retail_estimate?: number | null; expected_facebook_sale_price?: number | null;
  recommendation?: string | null; quantity?: number | null;
  analysis_json?: Record<string, unknown> | null; updated_at?: string | null;
};
...
function mapApiItem(item: ApiItem): Item {
  ...
  return {
    id: item.id, name: item.product_name, type: item.subcategory || "Pokémon sealed product",
    qty: Math.max(1, numberValue(item.quantity, 1)), cost, value: marketValue,
    trend: numberValue(analysis.trend30Day, 0), signal,
...
function App() {
  ...
  const loadPortfolio = async () => {
    try {
      ...
      if (items.length) {
        setInventory(items); setDataMode("database"); setMessage(`${items.length} products loaded from PostgreSQL.`);
      } else {
        setDataMode("sample"); setMessage("Database is connected, but no Pokémon inventory has been saved yet. Showing sample portfolio.");
      }
    } catch (error) {
      setDataMode("error");
  ...
  const refreshMarket = async () => {
    ...
    if (!liveItems.length) { setMessage("Save Pokémon inventory to PostgreSQL before running live market checks."); return; }
  ...
  const totals = useMemo(() => {
    ...
    const onlineNet = market * .86; const gain = market - invested;
    ...
    {scanMode && <div className="scan-backdrop" role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) closeScanner(); }}>
      <section className="scan-dialog" role="dialog" aria-modal="true" aria-labelledby="scan-title">
        ...
        <div className="scan-options">
          <button className="scan-option" onClick={() => cameraInput.current?.click()}><Camera size={24}/><span><b>Open camera</b><small>Take a new photo</small></span></button>
          <button className="scan-option" onClick={() => documentInput.current?.click()}><ImagePlus size={24}/><span><b>Choose file</b><small>Photo, image, or PDF</small></span></button>
        </div>
        {selectedFiles.length > 0 && <div className="selected-files"><b>{selectedFiles.length === 1 ? "Selected file" : `${selectedFiles.length} selected files`}</b>{selectedFiles.map((file, index) => <div className="selected-file" key={`${file.name}-${index}`}><FileImage size={17}/><span>{file.name}</span><small>{Math.max(1, Math.round(file.size / 1024))} KB</small></div>)}</div>}
